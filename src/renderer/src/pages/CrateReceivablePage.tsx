...
import { ChevronLeft, ChevronRight, RefreshCw, PackageCheck, Printer, ChevronUp, ChevronDown, ArrowUpDown, FilterX } from 'lucide-react'
...
interface BalanceRow {
  ...
  balance: number     // issued - received (positive = need to receive back)
...
export function CrateReceivablePage() {
  ...
  const loadBalances = async () => {
    ...
    try {
      ...
      // Process received entries (crates we got back)
      ...
      balanceMap.forEach((record, key) => {
        const balance = record.issued - record.received
        // For receivable page, only show positive balances (we need to get crates back)
        // or all if showZeroBalance is true
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    // Only show positive balances (receivable) unless showZeroBalance
    if (!showZeroBalance) {
  ...
  if (!activeCompany) {
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        ...
        <p className="text-muted-foreground">Please select a company to view crates receivable</p>
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex items-center justify-between border-b bg-white px-6 py-4">
        <div>
          <h1 className="text-2xl font-bold">Crate Receivable</h1>
          <p className="text-xs text-muted-foreground mt-1">Showing crates we issued and need to receive back</p>
        </div>
                    ...
                    {paginatedData.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={groupBy !== 'none' ? 4 : 5} className="text-center text-muted-foreground py-8">
                          {rows.length === 0 ? 'No data for selected date range' : 'No receivable balances found'}
                        </TableCell>
                      </TableRow>
                    ) : groupBy !== 'none' ? (
                      // Grouped view
                      (paginatedData as GroupedRow[]).map((group, idx) => (
                        <TableRow key={group.id}>
                          ...
                          <TableCell className={`text-right font-bold ${group.balance > 0 ? 'text-red-600' : group.balance < 0 ? 'text-green-600' : ''}`}>
                            {group.balance}
                          </TableCell>
                      ...
                      (paginatedData as BalanceRow[]).map((row, idx) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell className={`text-right font-bold ${row.balance > 0 ? 'text-red-600' : row.balance < 0 ? 'text-green-600' : ''}`}>
                            {row.balance}
                          </TableCell>
        ...
        <div className="w-auto p-4">
          <div className="flex gap-6 text-center">
            <div>
              ...
              <div className={`text-xl font-bold ${totalBalance > 0 ? 'text-red-600' : 'text-green-600'}`}>{totalBalance}</div>
            </div>
            <div>
