...
const SESSION_FROM_KEY = 'dailySaleRegister.fromDate'
const SESSION_TO_KEY = 'dailySaleRegister.toDate'
...
export function DailySaleRegisterPage() {
  ...
  const [voucherMeta, setVoucherMeta] = useState<Record<string, VoucherMeta>>({})
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('Failed to read daily sale register dates from session storage', error)
    }
    ...
    try {
      ...
      console.error('Failed to persist daily sale register dates to session storage', error)
    }
  }, [])
  ...
  const loadRegisterData = async () => {
    ...
    setLoading(true)
    try {
      const response = await window.api.voucher.listByDateRange(activeCompany.id, fromDate, toDate)
      if (response.success && Array.isArray(response.data)) {
        const flattened: RegisterItemRow[] = []
        const meta: Record<string, VoucherMeta> = {}

        response.data.forEach((voucher: any, voucherIndex: number) => {
          const voucherId = voucher.id
          const voucherDate = voucher.voucherDate ? String(voucher.voucherDate).split('T')[0] : ''
          const voucherNo = voucher.voucherNo || '-'
          const sellerName = voucher.supplierName || 'Unknown Seller'

          const charges: RegisterCharge[] = Array.isArray(voucher.charges)
            ? voucher.charges.map((charge: any, chargeIndex: number) => {
                ...
                return {
                  id: charge.id || `${voucherId}-charge-${chargeIndex}`,
                  label,
                  onValue,
                  per: perValue,
                  atRate,
          ...
          voucher.items?.forEach((item: any, itemIndex: number) => {
            const nug = Number(item.nug) || 0
            const weight = Number(item.weight) || 0
            const perUnit = (item.per || 'nug').toLowerCase()
            const rate = Number(item.customerPrice ?? item.customerRate ?? 0) || 0
            const supplierRate = Number(item.supplierPrice ?? item.supplierRate ?? 0) || 0
            const qtyForSeller = perUnit === 'kg' ? weight : nug
            const sellerAmount = supplierRate * qtyForSeller
            const expenses =
            ...
            flattened.push({
              id: `${voucherId}-${item.id || `${voucherIndex}-${itemIndex}`}`,
              voucherId,
              date: voucherDate,
              voucherNo,
              sellerId: voucher.supplierId,
              sellerName,
              ...
              cratePartyName: item.customerName || 'Unknown Customer'
            })
          })
        })
      ...
      console.error('Failed to load daily sale register', error)
      toast.error(error?.message || 'Failed to load daily sale register')
      setRows([])
      setVoucherMeta({})
    } finally {
      setLoading(false)
  ...
  const handleFromDateChange = (value: string) => {
    ...
    try {
      ...
      console.error('Failed to persist daily sale from date', error)
    }
  }
  ...
  const handleToDateChange = (value: string) => {
    ...
    try {
      ...
      console.error('Failed to persist daily sale to date', error)
    }
  }
  ...
  const saleSummaries = useMemo(() => {
    ...
    filteredRows.forEach((row) => {
      const meta = voucherMeta[row.voucherId]
      if (!summaryMap.has(row.voucherId)) {
      ...
      const summary = summaryMap.get(row.voucherId)!
    ...
    return summaries
  }, [filteredRows, voucherMeta, sortBy, sortDir])
  ...
  const generatePdfDocument = (dataset: ReturnType<typeof buildExportDataset>) => {
    ...
    doc.setFontSize(14)
    doc.text('Daily Sale Register', pageWidth / 2, yPos, { align: 'center' })
    yPos += 20
  ...
  const handlePreview = () => {
    ...
    try {
      ...
      console.error('Failed to preview daily sale register', error)
      toast.error(error?.message || 'Failed to preview daily sale register')
    ...
    try {
      if (type === 'excel' || type === 'csv') {
        ...
        if (type === 'excel') {
          ...
          XLSX.utils.book_append_sheet(workbook, worksheet, 'Daily Sale Register')
      ...
      toast.success(`Daily Sale register exported as ${type.toUpperCase()}`)
    } catch (error: any) {
      console.error('Failed to export daily sale register', error)
      toast.error(error?.message || 'Failed to export daily sale register')
    } finally {
      setIsExportModalOpen(false)
    }
  }
  ...
  if (!activeCompany) {
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        ...
        <p className="text-muted-foreground">Select a company to view the daily sale register</p>
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      <div className="flex items-center justify-between border-b bg-white px-6 py-4">
        <div>
          <h1 className="text-2xl font-bold">Daily Sale Register</h1>
          <p className="text-xs text-muted-foreground mt-1">
            Unified view of voucher items with buyer, seller, and crate level insights
          </p>
        </div>
                    ...
                    <TableBody>
                      {loading ? (
                        ...
                        <TableRow>
                          ...
                          >
                            {rows.length === 0 ? 'No daily sale entries for this period' : 'No rows match the filters'}
                          </TableCell>
                        </TableRow>
                      ) : groupBy !== 'none' ? (
                                    ...
                                    <div className="space-y-4 p-4">
                                      ...
                                      <div>
                                        ...
                                        {summary.charges.length > 0 ? (
                                          ...
                                          <p className="text-sm text-muted-foreground">No charges recorded for this voucher.</p>
                                        )}
                                      </div>
                                    </div>
                                  </TableCell>
      ...
      <ReportExportModal
        ...
        title="Export Daily Sale Register"
      />
      <ReportPreviewModal
        open={isPreviewOpen}
        title="Daily Sale Register Preview"
        pdfData={previewData}
