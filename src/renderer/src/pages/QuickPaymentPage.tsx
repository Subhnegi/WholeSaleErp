...
interface CreateQuickPaymentItemInput {
  paymentId: string
  accountId: string
  amount: number
  discount: number
...
type PaymentFormState = Omit<CreateQuickPaymentItemInput, 'paymentMode'> & {
  paymentMode: 'cash' | 'cheque' | 'upi' | 'banktransfer'
  accountName: string
}

const formatPaymentNumber = (sequence: number) => sequence.toString().padStart(3, '0')

const extractPaymentSequence = (paymentId?: string | null) => {
  if (!paymentId) return null
  const parsed = parseInt(paymentId, 10)
  return Number.isNaN(parsed) ? null : parsed
}

const computeNextPaymentNumber = (
  payments: QuickPayment[],
  fallbackSequence = 1
) => {
  const sequences = payments
    .flatMap(payment => payment.items || [])
    .map(item => extractPaymentSequence(item.paymentId))
    .filter((value): value is number => value !== null)
  const nextSequence = sequences.length > 0
    ? Math.max(...sequences) + 1
    : fallbackSequence
  return formatPaymentNumber(nextSequence)
}

const incrementPaymentNumber = (current?: string | null) => {
  const nextSequence = (extractPaymentSequence(current) || 0) + 1
  return formatPaymentNumber(nextSequence)
}
...
const createEmptyPaymentItem = (
  overrides: Partial<PaymentFormState> = {}
): PaymentFormState => ({
  paymentId: formatPaymentNumber(1),
  accountId: '',
  accountName: '',
  amount: 0,
  discount: 0,
...
interface ItemRow extends CreateQuickPaymentItemInput {
  tempId: string
  accountName: string
}
...
const normalizeItemRow = (row: LegacyItemRow, index: number): ItemRow => ({
  ...
  paymentId: row.paymentId || row.paymentNo || formatPaymentNumber(index + 1),
...
export function QuickPaymentPage({ tabId }: QuickPaymentPageProps) {
  ...
  const stateKey = `quickpayment-${tabId}`
  
  // Data lists
  const [, setAccounts] = useState<Account[]>([])
  const [customerAccounts, setCustomerAccounts] = useState<Account[]>([])
  
  // Shared Quick Payments data for cross-tab synchronization
  const [, setQuickPayments] = useState<QuickPayment[]>([])
  
  // Current entry form state
  const [paymentDate, setPaymentDate] = useState('')
  const [currentItem, setCurrentItem] = useState<PaymentFormState>(() => createEmptyPaymentItem())
  
  // Items added to current quick payment
  const [itemRows, setItemRows] = useState<ItemRow[]>([])
  const [savedItemRows, setSavedItemRows] = useState<ItemRow[]>([])
  const [nextTempId, setNextTempId] = useState(1)
  const [nextPaymentNumber, setNextPaymentNumber] = useState(() => formatPaymentNumber(1))
  ...
  // Editing state
  const [editingQuickPaymentId, setEditingQuickPaymentId] = useState<string | null>(null)
  ...
  const broadcastQuickPaymentSave = () => {
    console.log('[QuickPayment] Broadcasting save event to other tabs')
    const event = {
    ...
    localStorage.setItem('quickPaymentSync', JSON.stringify(event))
    setTimeout(() => {
      localStorage.removeItem('quickPaymentSync')
    }, 100)
  }
    ...
    if (prevTabIdRef.current !== tabId) {
      ...
      if (savedState) {
        try {
          ...
          setEditingQuickPaymentId(parsed.editingQuickPaymentId || null)
        } catch (error) {
  ...
  useEffect(() => {
    const state = {
      ...
      savedItemRows,
      paymentDate,
      editingQuickPaymentId
    }
    sessionStorage.setItem(stateKey, JSON.stringify(state))
  }, [itemRows, savedItemRows, paymentDate, editingQuickPaymentId, stateKey])
  ...
  useEffect(() => {
    broadcastChannelRef.current = new BroadcastChannel('quickpayment-sync')
    
    broadcastChannelRef.current.onmessage = (event) => {
      if (event.data.type === 'QUICK_RECEIPT_SAVED' && event.data.tabId !== tabId) {
        console.log('[QuickPayment] Received save event from another tab, refreshing...')
        loadQuickPayments()
      }
    }
  ...
  // Load quick payments
  const loadQuickPayments = async () => {
    if (!paymentDate || !activeCompanyId) return
    
    try {
      setLoading(true)
      const result = await window.api.quickPayment.listByCompany(activeCompanyId)
      if (result.success && result.data) {
        // Filter by date
        const filtered = result.data.filter(r => 
          toDateOnlyString(r.createdAt) === paymentDate
        )
        setQuickPayments(filtered as any)
        
        // Load saved items into the table (like QuickSale)
        if (filtered.length > 0 && shouldLoadRef.current) {
          const quickPaymentForDate = filtered[0] // Get the first payment for this date
          if (quickPaymentForDate && (quickPaymentForDate as any).items) {
            const loadedItems = (quickPaymentForDate as any).items.map((item: any, index: number) => ({
              tempId: `existing-${item.id}-${index}`,
              paymentId: item.paymentId || formatPaymentNumber(index + 1),
          ...
          // No saved payments for this date
          setEditingQuickPaymentId(null)
          setItemRows([])
          setSavedItemRows([])
          setSelectedRowIndex(null)
        ...
        const localMax = itemRows.reduce((max, row) => {
          const sequence = extractPaymentSequence(row.paymentId)
          return sequence && sequence > max ? sequence : max
        }, 0)
        const fallbackSequence = localMax + 1
        const nextNumber = computeNextPaymentNumber(filtered as any, fallbackSequence)
        setNextPaymentNumber(nextNumber)
        setCurrentItem(prev => ({
          ...prev,
          paymentId: nextNumber
        }))
      }
    } catch (error) {
      console.error('Failed to load quick payments:', error)
      toast.error('Failed to load quick payments')
    } finally {
      setLoading(false)
    }
  }
  
  // Load payments when date changes
  useEffect(() => {
    if (paymentDate && paymentDate !== prevDateRef.current) {
      prevDateRef.current = paymentDate
  ...
  // Keep current payment id in sync with counter
  useEffect(() => {
    setCurrentItem(prev => (
      prev.paymentId === nextPaymentNumber
        ? prev
        : { ...prev, paymentId: nextPaymentNumber }
    ))
  }, [nextPaymentNumber])
  ...
  const handleAddItem = () => {
    ...
    const itemToAdd: Omit<ItemRow, 'tempId'> = {
      paymentId: currentItem.paymentId,
    ...
    if (editingItemTempId) {
      ...
      const upcomingPaymentId = incrementPaymentNumber(nextPaymentNumber)
      setNextPaymentNumber(upcomingPaymentId)
      toast.success('Payment item added')
    }
    
    // Reset form
    const upcomingPaymentId = editingItemTempId ? nextPaymentNumber : incrementPaymentNumber(nextPaymentNumber)
    setCurrentItem(createEmptyPaymentItem({ paymentId: upcomingPaymentId }))
  }
  ...
  const handleDeleteItem = (tempId: string) => {
    ...
    toast.success('Payment item removed')
  }
  ...
  const editItemFromList = (tempId: string) => {
    ...
    setCurrentItem({
      paymentId: itemToEdit.paymentId,
    ...
    toast.info('Payment item loaded for editing')
  }
  
  // Clear form inputs
  const clearInputs = () => {
    setCurrentItem(createEmptyPaymentItem({ paymentId: nextPaymentNumber }))
    setEditingItemTempId(null)
  ...
  }, [currentItem, itemRows, paymentDate, selectedRowIndex, currentPage, itemsPerPage, selectedRows])
  
  // Save quick payment
  const handleSave = async () => {
    // Allow saving with 0 items only if we're editing (to delete the payment)
    if (itemRows.length === 0 && !editingQuickPaymentId) {
    ...
    try {
      ...
      if (editingQuickPaymentId) {
        // If editing and no items, delete the payment
        if (itemRows.length === 0) {
          result = await window.api.quickPayment.delete(editingQuickPaymentId)
          if (result.success) {
            toast.success('Quick payment deleted successfully')
          }
        } else {
          // Update existing payment
          result = await window.api.quickPayment.update(editingQuickPaymentId, {
            paymentDate,
            items: preparedItems
          })
          if (result.success) {
            toast.success('Quick payment updated successfully')
          }
        }
      } else {
        // Create new payment
        result = await window.api.quickPayment.create({
          companyId: activeCompanyId,
          paymentDate,
          items: preparedItems
        })
        if (result.success) {
          toast.success('Quick payment saved successfully')
        }
      }
      ...
      if (result.success) {
        ...
        broadcastQuickPaymentSave()
        
        // Enable loading for the reload
        shouldLoadRef.current = true
        
        // Reload payments to get the saved data
        await loadQuickPayments()
      } else {
        toast.error(result.error || 'Failed to save quick payment')
      }
    } catch (error) {
      console.error('Save error:', error)
      toast.error('Failed to save quick payment')
    } finally {
      setSubmitting(false)
    }
  }
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-12 gap-3 items-end">
                  ...
                  <div className="col-span-1">
                    <Label className="text-sm font-semibold mb-1">Payment No</Label>
                    <Input
                      value={currentItem.paymentId}
                      disabled
                      className="bg-muted"
                    />
                  </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-100">
                      ...
                      <TableHead className="font-bold">Payment No</TableHead>
                      ...
                      {paginatedRows.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={10} className="text-center text-muted-foreground py-8">
                            No payments added yet. Add payments using the form above.
                          </TableCell>
                        </TableRow>
                      ) : (
